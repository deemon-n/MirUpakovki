...
/*--------------------------------------------------------------*/
/* My functions to work with elements
/*--------------------------------------------------------------*/
...
// This function creates a new DOM element, specified by tagName
function $c(tagName, id, parentElement)
{
    var nel = document.createElement(tagName);
    if (id != "")
        nel.id = id;
    if (null != parentElement)
        parentElement.appendChild(nel);
    nel = $(nel);    
    return nel;
}

var useAddEventListener = document.documentElement.addEventListener;
var useAttachEvent = document.documentElement.attachEvent;

function getCurrentEventTarget(ev)
{
    var targ = null;
    if (!ev) 
        var ev = window.event;
    if (ev.currentTarget) 
        ...
        if (ev.srcElement) targ = ev.srcElement;
...
function attachEcmaClass(domElement, ecmaClass)
{
    domElement._ecmaClass = ecmaClass;
}
// возвращает в контексте события соответствующий класс
function getEcmaClass(ev)
{
    var fEl = getCurrentEventTarget(ev);
    while (fEl.parentNode)
    {
        if (fEl._ecmaClass)
            return fEl._ecmaClass;
        fEl = fEl.parentNode;
    }
    return null;
}
// возвращает родительский объект по ecma-классу
function getParentNode(el, ecmaClass)
{
    while (el.parentNode)
    {
        if (el._ecmaClass == ecmaClass) 
            return el;
        el = el.parentNode;
    }
    return null;
} 
// Converts CSS "px"-values to Integer 
function evalCSSValue(value)
{
    var param = value.split("px");
    return eval(param[0])
}